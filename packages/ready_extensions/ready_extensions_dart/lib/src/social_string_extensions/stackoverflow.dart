...
class StackoverflowQuestionUrl {
  ...
  static StackoverflowQuestionUrl? parse(String url) {
    var matches = RegExp(r'^(?:https?:)?\/\/(?:www\.)?stackoverflow\.com\/questions\/(?<id>[0-9]+)\/(?<title>[A-z0-9-_.]+)\/?$').allMatches(url);
...
class StackoverflowUserUrl {
  ...
  static StackoverflowUserUrl? parse(String url) {
    var matches = RegExp(r'(?:https?:)?\/\/(?:www\.)?stackoverflow\.com\/users\/(?<id>[0-9]+)\/(?<username>[A-z0-9-_.]+)\/?').allMatches(url);
    var _id = matches.getValue("id");
