...
class FacebookProfileUrl {
  ...
  static FacebookProfileUrl? parse(String url) {
    var matches = RegExp(r'(?:https?:)?\/\/(?:www\.)?(?:facebook|fb)\.com\/(?<profile>(?![A-z]+\.php)(?!marketplace|gaming|watch|me|messages|help|search|groups)[A-z0-9_\-\.]+)\/?').allMatches(url);
...
class FacebookProfileIdUrl {
  ...
  static FacebookProfileIdUrl? parse(String url) {
    var matches = RegExp(r'(?:https?:)?\/\/(?:www\.)facebook.com/(?:profile.php\?id=)?(?<id>[0-9]+)').allMatches(url);
