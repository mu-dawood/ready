...
extension CurrentDataExt<T> on CurrentData<T> {
  ...
  CurrentData<T> add(T value,
    ...
    return copyWith(
      items: data,
      totalCount: totalCountResolver(totalCount, 1),
    );
  }
  ...
  /// The list must be growable.
  CurrentData<T> addAll(Iterable<T> iterable,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
    data.addAll(items);
    return copyWith(
      items: data,
      totalCount: totalCountResolver(totalCount, iterable.length),
    );
  }
  ...
  CurrentData<T> insert(int index, T element,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
  ...
  /// The list must be growable.
  /// The [index] value must be non-negative and no greater than [length].
  CurrentData<T> insertAll(int index, Iterable<T> iterable,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
  ...
  CurrentData<T> remove(T value,
    ...
    return copyWith(
      items: data,
      totalCount: totalCountResolver(totalCount, diff(oldLength, newLength)),
    );
  }
  ...
  ///
  /// The list must be growable.
  CurrentData<T> removeRange(int start, int end,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
    var oldLength = data.length;
  ...
  ///
  /// The list must be growable.
  CurrentData<T> removeWhere(bool Function(T) test,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
    var oldLength = data.length;
  ...
  ///
  /// The list must be growable.
  CurrentData<T> removeAt(int index,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    if (index >= items.length) return this;
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
    var oldLength = data.length;
    data.removeAt(index);
  ...
  CurrentData<T> where(bool Function(T) test,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var current = items;
    List<T> data = current is List<T> ? current : current.toList();
    var oldLength = data.length;
  ...
  CurrentData<T> skip(int count,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.skip(count);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
    );
  }
  ...
  CurrentData<T> skipWhile(bool Function(T) test,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.skipWhile(test);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
  ...
  CurrentData<T> take(int count,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.take(count);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
  ...
  CurrentData<T> takeWhile(bool Function(T) test,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.takeWhile(test);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
  ...
  ///   }
  /// }
  CurrentData<T> expand(Iterable<T> Function(T) toElements,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.expand(toElements);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
  ...
  /// original iterables.
  CurrentData<T> followedBy(Iterable<T> other,
      [TotalCountResolver totalCountResolver = _defaultResolver]) {
    var oldLength = items.length;
    var newData = items.followedBy(other);
    return copyWith(
      items: newData,
      totalCount:
          totalCountResolver(totalCount, diff(oldLength, newData.length)),
  ...
  ///   }
  /// }
  CurrentData<T> map(T Function(T) toElement) {
    return copyWith(items: items.map(toElement));
  }

  /// replace items where test return `true`
  CurrentData<T> replaceWhere(
      {required bool Function(T) where, required T item}) {
    return copyWith(items: items.map((old) => where(old) ? item : old));
  }
  ...
  CurrentData<T> clear(
    ...
    return copyWith(
      items: [],
      totalCount: totalCountResolver(totalCount, diff(items.length, 0)),
    );
  }
}
